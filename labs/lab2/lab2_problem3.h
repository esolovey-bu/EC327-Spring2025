...
/**
* Helper struct to pass along an array of characters representing a string as well as their size.
*/
...
* happen after the substring has been computed not before (this will be obvious from
* the examples below).
*
...
*
* Note that the return type of the function is std::string, a C++ style string, while the input is an
* array of characters (equivalent to char*), which is a C-style string.  We will do a lot more with
* C-style strings later, but for this probem please be aware of the following things:
*
*  1) you can access C-style strings like you would any other array
*  2) You can concatenate a C-style string to a C++ string with cString += cStyleString;
*  3) For above to work as expected, the cStyleString should have a null character, '\0',
*  as its last character.  So, when building the C-style string, allocate one extra space
*  in the array and set the last elemnent to '\0'.
*/
